...
test.describe('Dashboard 7: Modern APT Overview - APT Detail Modal', () => {
  ...
  test('Modal displays comprehensive APT profile information', async ({ page }) => {
    ...
    await expect(
      modalBody.locator('.modal-section-title').filter({ hasText: 'Overview' })
    ).toBeVisible();

    // Verify Motivation section
    await expect(
      modalBody.locator('.modal-section-title').filter({ hasText: 'Motivation' })
    ).toBeVisible();

    // Verify Targeted Sectors section
    await expect(
      modalBody.locator('.modal-section-title').filter({ hasText: 'Targeted Sectors' })
    ).toBeVisible();

    // Verify Targeted Countries section
    await expect(
      modalBody.locator('.modal-section-title').filter({ hasText: 'Targeted Countries' })
    ).toBeVisible();

    // Verify MITRE ATT&CK Techniques section
    await expect(
      modalBody.locator('.modal-section-title').filter({ hasText: 'MITRE ATT&CK Techniques' })
    ).toBeVisible();

    // Verify Malware Arsenal section
    await expect(
      modalBody.locator('.modal-section-title').filter({ hasText: 'Malware Arsenal' })
    ).toBeVisible();

    // Verify External References section
    await expect(
      modalBody.locator('.modal-section-title').filter({ hasText: 'External References' })
    ).toBeVisible();
  });
  ...
  test('Modal displays malware information correctly', async ({ page }) => {
    ...
    const modalBody = page.locator('#modalBody');
    const malwareSection = modalBody
      .locator('.modal-section-title')
      .filter({ hasText: 'Malware Arsenal' });
